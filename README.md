...
**无需特定命名**：程序会自动读取该文件夹下的所有图片（支持 jpg, png, webp 等格式）。
建议：使用正方形或 4:3 比例的图片，文件大小不宜过大（建议单张 500kb 以内以保证流畅度）。

*(注：顶部的星星是 3D 程序化生成的，不需要 `top.jpg`)*

### 2. 替换照片
直接将你自己的照片复制到 `public/photos/` 文件夹中，覆盖或删除原有的图片即可。
无需修改代码，照片数量会自动调整。

### 3. 音乐配置
同样，将音乐文件放入 public/music/ 文件夹即可自动加载。
### 🖐️ 手势控制说明
